...
-- Crear usuarios de prueba con limpieza más específica
DO $$
DECLARE
    basic_user_id UUID;
    pro_user_id UUID;
...
BEGIN
    ...
    -- Generar IDs específicos que no chocarán
    basic_user_id := 'aaaaaaaa-0001-0001-0001-000000000001'::UUID;
    pro_user_id := 'aaaaaaaa-0002-0002-0002-000000000002'::UUID;
    demo_user_id := 'aaaaaaaa-0003-0003-0003-000000000003'::UUID;
    
    -- Crear usuarios en auth.users con campos mínimos requeridos
    INSERT INTO auth.users (
        ...
        (basic_user_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'admin_basic@opobots.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"full_name": "Admin Básico"}'::jsonb),
        (pro_user_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'admin_pro@opobots.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"full_name": "Admin Profesional"}'::jsonb),
        (demo_user_id, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'admin_demo@opobots.com', crypt('password123', gen_salt('bf')), now(), now(), now(), '{"full_name": "Admin Demo"}'::jsonb);
    ...
    RAISE NOTICE 'Usuarios de prueba creados exitosamente con IDs fijos:';
